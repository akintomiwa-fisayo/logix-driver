...
export default class DriverCompleteTrip extends React.Component {
  ...
  _getLocatioAsync = async () => {
    if (this.state.status == 'START' && this.state.loadingModal == false) {
      ...
      return fetch('https://maps.googleapis.com/maps/api/geocode/json?latlng=' + latlng + '&key=' + google_map_key)
        ...
        .then((responseJson) => {
          console.log("3", { location, responseJson: Object.keys(responseJson.results[0]) })
  ...
  callToCustomer(data) {
    // console.log("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",{data})
    if (data.recieverPhone) {
      Linking.canOpenURL('tel:' + data.recieverPhone).then(supported => {
        if (!supported) {
          console.log('Can\'t handle Phone Number: ' + data.recieverPhone);
        } else {
          return Linking.openURL('tel:' + data.recieverPhone);
        }
      })
    }else{
