...
export default class DriverTripAccept extends React.Component {
    ...
    onPressAccept(item) {
        const userRef = firebase.database().ref('users/' + this.state.curUid);
        try {
            userRef.on('value', (userData) => {
                console.log("the on accept delivery click", { item, userData })
                ...
                var data = {
                    ...
                    driver_manager: userData.manager || null,
                ...
                let dbRef = firebase.database().ref('users/' + this.state.curUid + '/my_bookings/' + item.bookingId + '/'); dbRef.update(data).then(() => {
